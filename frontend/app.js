...
if (isRecvPage) {
  ...
  otpWrap.addEventListener("paste", (e) => {
    ...
    if (text.length >= otpInputs.length) joinBtn.click();
  });
...
async function createPeer() {
  ...
  pc.onicecandidate = (e) => {
    if (e.candidate) ws.send(JSON.stringify({ type: "ice", candidate: e.candidate, roomId }));
  };
  ...
  return new Promise((resolve) => {
    ...
    reader.onload = (e) => {
      const sendChunk = () => {
        ...
        overallSendBar.style.width = overallPct + "%";
...
function setupReceiver() {
  ...
  channel.onmessage = (e) => {
    ...
    if (currentFile && currentRow) {
      ...
      overallRecvBar.style.width = overallPct + "%";
    }
